...
-- producent ze srednio najdro¿szymi drukarkami atramentowymi lol innym sposobem zrób wtf

select Produkty.producent as Producent, avg(Drukarki.cena) as Cena
from Drukarki 
join Produkty on Produkty.model = Drukarki.model
where Drukarki.typ = 'atramentowa'
group by Produkty.producent
order by avg(Drukarki.cena) desc
